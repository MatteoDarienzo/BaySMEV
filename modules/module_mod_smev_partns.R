...
################################################################################  
smev_partns= function(algorithm = "fminsearch",  #"fminsearch","rstan","cmdstan","adaptMCMC"
                      Nmcmc=4000,
                      Nmcmc_max=40000,
                      nburn=0.5,
                      Nslim=1, 
                      scales=c(0.02,0.1,0.005),
                      priors_id=list(),
                      acc.rate=0.234,
                      y=c(), 
                      t=c(), 
                      ...
                      ...){
################################################################################
  #^* GOAL: estimation of SMEV parameters PARTIAL-NON-STATIONARY MODEL         #
  #^*       only scale depends linearly on t, while shape is fixed             #
  #^***************************************************************************#
  ...
  #^****************************************************************************
    ...
    fmins=NULL
    initval=c(priors_id$shape_intercept[[4]], # shape interc start value
              priors_id$scale_intercept[[4]], # scale interc start value
              priors_id$scale_slope[[4]])     # scale slope start value
    fmins=pracma::fminsearch(f=logLike1_partns,
                             x0=initval, 
                             y=y,
                             t=t,
                             thr=thr,
  ...
  #^****************************************************************************
    ...
    ###############
      ...
      while ((converg ==F) & (Nmcmc_tmp < Nmcmc_max)){
        it=it+1
        set.seed(1)
        # mcmc_Lik=metrop(obj=logLike, initial=start_values, scale=scales, 
        #                 nbatch=Nmcmc_tmp, y=y, t=t, thr=thr)
        mcmc_Lik=adaptMCMC::MCMC(logLike_partns, 
                                 n               = Nmcmc_tmp, 
                                 init            = c(priors_id$shape_intercept[[4]], 
                                                     priors_id$scale_intercept[[4]], 
                                                     priors_id$scale_slope[[4]]),
                                 scale           = c(scales[1],scales[3],scales[4]), 
                                 adapt           = T,
                                 showProgressBar = F,
                                 acc.rate        = acc.rate, 
                                 y               = y, 
        ...
        converg=mcmc.diagn_partns(zPost       =zLik, 
                                  priors      =NULL, 
                                  dir_res     =dir.res_id,  
      ...
      # scale slope
      s_tmp$b_C_quant_Like=quantile(zLik[,3],probs=c(0.05,0.5,0.95))
    }
    
    
    # Compute also the fminsearch() MLE (as Marra et al., 2019) for comparison:
    fmins=NULL
    initval=c(priors_id$shape_intercept[[4]], # shape interc start value
              priors_id$scale_intercept[[4]], # scale interc start value
              priors_id$scale_slope[[4]])     # scale slope start value
    ...
    #################  
      ...
      while ((converg==F) & (Nmcmc_tmp<Nmcmc_max)){
        ...
        # mcmc_Post=metrop(obj=logPost, initial=start_values, scale=scales, 
        #                  nbatch=Nmcmc_tmp, y=y, t=t, thr=thr)
        mcmc_Post=adaptMCMC::MCMC(logPost_partns, 
                                  n               = Nmcmc_tmp, 
                                  init            = c(priors_id$shape_intercept[[4]], 
                                                      priors_id$scale_intercept[[4]], 
                                                      priors_id$scale_slope[[4]]),
        ...
        converg=mcmc.diagn_partns(zPost       = zPost, 
                                  priors      = priors_id, 
                                  dir_res     = dir.res_id,  
      ...
      # compute Posterior likelihood to compute the performance criteria:
      marg.like=c()
      for (tt in 1:nrow(zPost)){
        marg.like[tt]=logLike_partns(theta=zPost[tt,],y=y,t=t,thr=thr)
      }
      # compute maxpost:
      s_tmp$theta_maxpost=zPost[which.max(zPost_p),]
      s_tmp$maxpost=max(zPost_p)
      
      # criterion using likelihood or posterior (different versions):
      # DIC: Gelman et al, 2004 "Bayesian data analysis"
      all_dic=calc.dic(zPost=zPost,lik=marg.like,post=zPost_p,
                       lik.fun=logLike_partns,t=t,y=y,thr=thr)
      ...
      # AIC: Aikake, 1974
      all_aic=calc.aic(zPost=zPost,lik=marg.like,post=zPost_p,y=y,t=t,thr=thr)  
      s_tmp$AIC=all_aic$AIC
      s_tmp$AIC_b=all_aic$AIC_b
      # BIC: Schwarz, 1978
      all_bic=calc.bic(zPost=zPost,lik=marg.like,post=zPost_p,y=y,t=t,thr=thr)
      s_tmp$BIC=all_bic$BIC
      s_tmp$BIC_b=all_bic$BIC_b
      # HQC: Hannan-Quinn, 1979
      all_hqc=calc.hqc(zPost=zPost,lik=marg.like,post=zPost_p,y=y,t=t,thr=thr)
      ...
      # scale slope
      s_tmp$b_C_quant_Post=quantile(zPost[,3],probs=c(0.05,0.5,0.95))
    }
  ...
  #^**************************************************************************
    ...
    smodel=paste0("functions {
        vector rep(real k, int N) {
           vector[N] c;
           for (n in 1:N){
              c[n] = k;
           }
      ...
      model {
        ...
        target += weibull_lpdf(y_uncen | alpha, sigma_0+sigma_1*t_uncen);
        target += weibull_lcdf(thr | alpha, sigma_0+sigma_1*t_cen);
      }")
    
    file<-file.path(cmdstan_path(),"examples","smev")
    f<-write_stan_file(smodel,
                       dir=file,
                       basename=paste0("partns_smev_dur",durations[dur]/60,"h.stan"))
    mod=0
    mod<-cmdstan_model(f)
    ...
    fit2=0
    fit2<-rstan::stan(
      file=paste0("partns_smev_dur",durations[dur]/60,"h.stan"),  # Stan program
    ...
    rstan::traceplot(fit2,
                     pars=c("alpha_0","sigma_0","sigma_1"),
                     inc_warmup=T,nrow=2)
    # scatterplots with correlations:
    pairs(fit2,pars=c("alpha_0","sigma_0","sigma_1"),las=2)
  ...
  #^****************************************************************************
    ...
    # Bayesian Inference:
    #####################
    print("Apply cmdstanr package (Hamiltonian mcmc)...")
    if (flag_like){
      # perform also MAX LIKELIHOOD to be compared with MAXPOST:
      # print("Max Likelihood")
      converg=F
      # NOT DONE YET!!!
      # ...
    ...
    if (flag_bayes){
      ...
      model {
        ...
        target += weibull_lpdf( y_uncen | rep(alpha,N_uncen), sigma_0+sigma_1*t_uncen);
        target += weibull_lcdf(thr | rep(alpha,N_cen), sigma_0+sigma_1*t_cen);
      }")
      ...
      init_f_stat<-function () list(alpha=rlnorm(1,priors_id$shape_intercept[[2]],0.1),
                                    sigma_0=rlnorm(1,priors_id$scale_intercept[[2]],0.1),
                                    sigma_1=rnorm(1,priors_id$scale_slope[[2]],0.01))
      fit=0
      nchain=0
      Nmcmc_tmp=Nmcmc
      while (nchain<4){
        ...
        # fit$summary(variables=c("alpha","sigma0","sigma1"),"mean","sd")
    ...
    zPost=cbind(list_of_draws$alpha,
                list_of_draws$sigma_0,
                list_of_draws$sigma_1)
    zPost_p=list_of_draws$lp__
    zPost=apply(zPost,2,function(x) x[seq(1,length(x),1)])  # thinning
    ...
    marg.like=c()
    for (tt in 1:nrow(zPost)){
      marg.like[tt]=logLike_partns(theta=zPost[tt,],
                                   y=y,
                                   t=t,
                                   thr=thr)
    }
